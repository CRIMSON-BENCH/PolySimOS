...
export function GraphTraversalStudio() {
  ...
  // Recompute the BFS/DFS visit order whenever the mode changes, and restart the walk.
  useEffect(() => {
    ...
    orderRef.current = order; stepRef.current = 0;
  }, [mode]);
  ...
  const frame = (steps: number) => {
    ...
    if (stepRef.current >= order.length) stepRef.current = 0; // loop back to the start
    stepRef.current = Math.min(order.length, stepRef.current + steps);
    const step = stepRef.current;
    const visited = new Set(order.slice(0, step)); const current = order[step - 1];
    const ctx = hidpi(canvas, 560, 360); ctx.fillStyle = "#020617"; ctx.fillRect(0, 0, 560, 360);
  ...
  return (
    <StudioChrome title="Graph Traversal (BFS / DFS)" tagline="breadth vs depth"
      controls={<div>
        ...
        <TransportBar playing={t.playing} onToggle={t.toggle} onStep={t.step} onReset={() => { stepRef.current = 0; t.step(); }} speed={t.speed} onSpeed={t.setSpeed} />
