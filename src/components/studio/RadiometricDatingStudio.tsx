...
export function RadiometricDatingStudio() {
  ...
  const [fraction, setFraction] = useState(0.5); // remaining fraction
  ...
  return (
    <StudioChrome title="Radiometric Dating" tagline="reading the atomic clock"
      controls={<div>
        ...
        <Slider label="Remaining fraction" value={fraction} min={0.01} max={0.99} step={0.01} onChange={setFraction} />
      ...
      inspector={<div><Stat label="Half-life" value={`${hl.toExponential(2)} ${ISO[iso].unit}`} /><Stat label="Estimated age" value={ageStr} /><Stat label="Best for" value={ISO[iso].use} /></div>}
