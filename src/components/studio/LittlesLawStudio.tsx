...
export function LittlesLawStudio() {
  ...
  const throughputRef = useRef(throughput); throughputRef.current = throughput;
  const wipRef = useRef(wip); wipRef.current = wip;
  const tRef = useRef(0);

  const frame = (steps: number) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    for (let s = 0; s < steps; s++) tRef.current += 0.02;
    const t = tRef.current; const throughput = throughputRef.current; const wip = wipRef.current;
    const W = 520, H = 260; const ctx = hidpi(canvas, W, H); ctx.fillStyle = "#020617"; ctx.fillRect(0, 0, W, H);
    // pipe
