...
export function VaRStudio() {
  ...
  const [annVol, setAnnVol] = useState(0.2);
  const [annRet, setAnnRet] = useState(0.07);
  ...
  return (
    <StudioChrome title="Value at Risk (VaR)" tagline="parametric VaR & expected shortfall"
      controls={<div>
        <Slider label="Portfolio value ($)" value={value} min={10000} max={10000000} step={10000} onChange={setValue} />
        <Slider label="Annual volatility" value={annVol} min={0.05} max={0.6} step={0.01} onChange={setAnnVol} />
        <Slider label="Annual return" value={annRet} min={-0.1} max={0.2} step={0.01} onChange={setAnnRet} />
        <Slider label="Horizon (days)" value={days} min={1} max={60} step={1} onChange={setDays} />
      ...
      inspector={<div><Stat label={`VaR (${conf}%)`} value={`$${VaR.toLocaleString(undefined, { maximumFractionDigits: 0 })}`} /><Stat label="Expected shortfall" value={`$${CVaR.toLocaleString(undefined, { maximumFractionDigits: 0 })}`} /><Stat label="Horizon vol" value={`${(horizonVol * 100).toFixed(2)}%`} /></div>}
