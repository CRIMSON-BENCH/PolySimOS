...
export function BlackScholesStudio() {
  ...
  return (
    <StudioChrome title="Black-Scholes Option Pricing" tagline="fair value + the Greeks"
      controls={<div>
        <Slider label="Spot price S" value={S} min={20} max={200} step={1} onChange={setS} />
        <Slider label="Strike K" value={K} min={20} max={200} step={1} onChange={setK} />
        <Slider label="Time to expiry (yr)" value={T} min={0.02} max={2} step={0.02} onChange={setT} />
        <Slider label="Risk-free rate" value={r} min={0} max={0.1} step={0.005} onChange={setR} />
      ...
      inspector={<div><Stat label="Call price" value={`$${call.toFixed(2)}`} /><Stat label="Put price" value={`$${put.toFixed(2)}`} /><Stat label="Delta (call)" value={delta.toFixed(3)} /><Stat label="Gamma" value={gamma.toFixed(4)} /><Stat label="Vega (per 1%)" value={vega.toFixed(3)} /><Stat label="Theta (per day)" value={theta.toFixed(3)} /></div>}
