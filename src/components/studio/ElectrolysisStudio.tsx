...
export function ElectrolysisStudio() {
  ...
  const [metal, setMetal] = useState(0), [current, setCurrent] = useState(2), [minutes, setMinutes] = useState(30);
  ...
  return (
    <StudioChrome title="Electrolysis (Faraday's Laws)" tagline="electroplating by the numbers"
      controls={<div>
        ...
        <Slider label="Current (A)" value={current} min={0.1} max={10} step={0.1} onChange={setCurrent} />
        <Slider label="Time (minutes)" value={minutes} min={1} max={240} step={1} onChange={setMinutes} />
