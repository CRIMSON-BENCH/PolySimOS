...
export function BondDurationStudio() {
  ...
  const [coupon, setCoupon] = useState(5), [ytm, setYtm] = useState(5), [years, setYears] = useState(10), [face, setFace] = useState(1000);
  ...
  return (
    <StudioChrome title="Bond Duration & Price" tagline="interest-rate sensitivity"
      controls={<div>
        <Slider label="Coupon rate (%)" value={coupon} min={0} max={12} step={0.25} onChange={setCoupon} />
        <Slider label="Yield to maturity (%)" value={ytm} min={1} max={12} step={0.25} onChange={setYtm} />
        <Slider label="Years to maturity" value={years} min={1} max={30} step={1} onChange={setYears} />
