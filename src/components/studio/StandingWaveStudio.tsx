...
export function StandingWaveStudio() {
  ...
  const frame = (steps: number) => {
    ...
    for (let px = pad; px <= W - pad; px++) { const x = (px - pad) / (W - 2 * pad); const y = amp * Math.sin(mode * Math.PI * x) * Math.cos(phase.current); px === pad ? ctx.moveTo(px, mid - y) : ctx.lineTo(px, mid - y); } ctx.stroke();
    // nodes
