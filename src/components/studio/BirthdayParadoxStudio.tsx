...
print("pairs", group * (group - 1) // 2)`;
  ...
  return (
    <StudioChrome title="Birthday Paradox" tagline="smaller than you'd think"
      ...
      inspector={<div><Stat label="Group size" value={String(group)} /><Stat label="P(shared birthday)" value={`${(cur * 100).toFixed(1)}%`} /><Stat label="Pairs" value={String(pairs)} /><ExplainResult text={explain} /></div>}
