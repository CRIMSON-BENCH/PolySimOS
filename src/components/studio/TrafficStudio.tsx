...
export function TrafficStudio() {
  ...
  const pSlowRef = useRef(pSlow); pSlowRef.current = pSlow;
  ...
  const frame = (steps: number) => {
    ...
    const step = () => {
      ...
      const nv = new Array(CELLS).fill(-1); let moved = 0, cars = 0;
      for (let i = 0; i < CELLS; i++) { if (l[i] < 0) continue; cars++; let v = Math.min(l[i] + 1, MAXV); v = Math.min(v, gap(i)); if (v > 0 && Math.random() < pSlowRef.current) v--; nv[(i + v) % CELLS] = v; moved += v; }
      lane.current = nv;
