...
export function PredatorPreyStudio() {
  ...
  const preyReproRef = useRef(preyRepro); preyReproRef.current = preyRepro;
  const predStarveRef = useRef(predStarve); predStarveRef.current = predStarve;
  ...
  const frame = (steps: number) => {
    ...
    const preyRepro = preyReproRef.current, predStarve = predStarveRef.current;
    ...
    if (frameRef.current++ % 8 === 0) setCounts({ prey, pred });
  };
...
print("seed:", (g==PREY).sum(), "prey", (g==PRED).sum(), "predators")`;
  ...
  return (
    <StudioChrome title="Spatial Predator–Prey" tagline="agent-based ecology on a grid"
      controls={<div>
        <TransportBar playing={tr.playing} onToggle={tr.toggle} onStep={tr.step} onReset={() => { seed(); tr.step(); }} speed={tr.speed} onSpeed={tr.setSpeed} />
