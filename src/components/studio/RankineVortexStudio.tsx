...
export function RankineVortexStudio() {
  ...
  const coreRRef = useRef(coreR); coreRRef.current = coreR;
  const maxWindRef = useRef(maxWind); maxWindRef.current = maxWind;
  const rot = useRef(0);
  ...
  const frame = (steps: number) => {
    ...
    if (!canvas) return;
    const coreR = coreRRef.current, maxWind = maxWindRef.current;
    const windAt = (r: number) => r <= coreR ? maxWind * (r / coreR) : maxWind * (coreR / r);
    rot.current += 0.05 * steps; const W = 520, H = 300; const ctx = hidpi(canvas, W, H); ctx.fillStyle = "#020617"; ctx.fillRect(0, 0, W, H);
