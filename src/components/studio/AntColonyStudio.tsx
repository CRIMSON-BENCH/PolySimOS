...
export function AntColonyStudio() {
  ...
  const nAntsRef = useRef(nAnts); nAntsRef.current = nAnts;
  const evapRef = useRef(evap); evapRef.current = evap;
  const rngRef = useRef(4242);
  ...
  const frame = (steps: number) => {
    ...
    for (let stp = 0; stp < steps; stp++) {
      ...
      for (let k = 0; k < Math.round(nAntsRef.current); k++) {
      ...
      for (let i = 0; i < M; i++) for (let j = 0; j < M; j++) ph[i][j] = ph[i][j] * (1 - evapRef.current) + deposit[i][j];
      if (!bestTour.current.length || roundBestLen < tourLen(bestTour.current)) bestTour.current = roundBest;
    }
    setBestLen(tourLen(bestTour.current)); setIter((n) => n + steps);
    const M = cities.current.length; const ph = phero.current;
    const ctx = hidpi(canvas, W, H); ctx.fillStyle = "#0b1220"; ctx.fillRect(0, 0, W, H);
