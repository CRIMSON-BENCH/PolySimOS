...
export function GeostrophicWindStudio() {
  ...
  const [gradient, setGradient] = useState(3); // mbar per 100 km
  ...
  return (
    <StudioChrome title="Geostrophic Wind" tagline="wind parallel to isobars"
      controls={<div>
        <Slider label="Pressure gradient (mbar/100km)" value={gradient} min={0.5} max={8} step={0.5} onChange={setGradient} />
        <Slider label="Latitude (°)" value={latitude} min={5} max={90} step={1} onChange={setLatitude} />
      ...
      inspector={<div><Stat label="Geostrophic wind" value={isFinite(vg) ? `${vg.toFixed(1)} m/s` : "∞ (equator)"} /><Stat label="In km/h" value={isFinite(vg) ? `${(vg * 3.6).toFixed(0)} km/h` : "—"} /><Stat label="Coriolis f" value={`${(f * 1e4).toFixed(2)}×10⁻⁴`} /></div>}
