...
export function LinearRegressionStudio() {
  ...
  const [nPts, setNPts] = useState(40);
  ...
  return (
    <StudioChrome title="Linear Regression" tagline="least squares & R²"
      controls={<div>
        <Slider label="True slope" value={slope} min={-3} max={3} step={0.1} onChange={setSlope} />
        <Slider label="Noise level" value={noise} min={0} max={8} step={0.5} onChange={setNoise} />
        <Slider label="Data points" value={nPts} min={5} max={150} step={5} onChange={setNPts} />
      ...
      inspector={<div><Stat label="Fitted slope" value={fit.m.toFixed(3)} /><Stat label="Intercept" value={fit.b.toFixed(3)} /><Stat label="R²" value={fit.r2.toFixed(3)} /><Stat label="Correlation r" value={(Math.sign(fit.m) * Math.sqrt(Math.max(0, fit.r2))).toFixed(3)} /></div>}
