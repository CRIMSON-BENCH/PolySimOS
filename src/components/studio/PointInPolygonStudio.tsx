...
export function PointInPolygonStudio() {
  ...
  useCanvasDrag(canvasRef, W, H, {
    pick: (x, y) => { didDrag.current = false; return Math.hypot(pt[0] - x, pt[1] - y) < 16; },
    move: (x, y) => { didDrag.current = true; setPt([x, y]); },
  });
  ...
  useEffect(() => {
    ...
    ctx.fillStyle = "#94a3b8"; ctx.font = "11px sans-serif"; ctx.fillText(`ray crosses edges ${cross} times → ${cross % 2 ? "inside" : "outside"}`, 10, 20); ctx.fillText("drag the point, or click elsewhere to move it", 10, 350);
  }, [pt, inside]);
...
print(inside(poly, ${pt[0].toFixed(0)}, ${pt[1].toFixed(0)}))`;
  ...
  return (
    ...
    ><canvas ref={canvasRef} width={500} height={360} onClick={(e) => {
        if (didDrag.current) { didDrag.current = false; return; } // a drag just ended — don't also teleport
        const r = (e.target as HTMLCanvasElement).getBoundingClientRect(); setPt([(e.clientX - r.left) * 500 / r.width, (e.clientY - r.top) * 360 / r.height]);
