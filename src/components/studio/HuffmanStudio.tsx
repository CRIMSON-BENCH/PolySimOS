...
export function HuffmanStudio() {
  ...
  return (
    <StudioChrome title="Huffman Coding" tagline="optimal lossless compression"
      ...
      inspector={<div><Stat label="Symbols" value={String(entries.length)} /><Stat label="Huffman bits" value={String(huffBits)} /><Stat label="Fixed-length bits" value={String(fixedBits)} /><Stat label="Compression" value={`${(ratio * 100).toFixed(0)}%`} /></div>}
    ><div className="p-4">
