...
export function MarathonPacingStudio() {
  ...
  const [distance, setDistance] = useState(42.2); // km
  const [targetMin, setTargetMin] = useState(240); // min
  ...
  return (
    <StudioChrome title="Marathon Pacing" tagline="hit your goal time"
      controls={<div>
        <div className="mb-3 grid grid-cols-3 gap-1">{[["10K", 10], ["Half", 21.1], ["Full", 42.2]].map(([n, d]) => <button key={n as string} onClick={() => setDistance(d as number)} className={`rounded-lg px-1 py-1 text-xs font-semibold ${distance === d ? "bg-cyan-600 text-white" : "border border-slate-300 text-slate-600 dark:border-slate-700 dark:text-slate-400"}`}>{n}</button>)}</div>
        <Slider label="Target time (min)" value={targetMin} min={30} max={360} step={5} onChange={setTargetMin} />
      ...
      inspector={<div><Stat label="Average pace" value={`${paceMin}:${paceSec.toString().padStart(2, "0")}/km`} /><Stat label="Speed" value={`${kmh.toFixed(1)} km/h`} /><Stat label="Finish time" value={`${Math.floor(targetMin / 60)}h ${(targetMin % 60).toFixed(0)}m`} /></div>}
