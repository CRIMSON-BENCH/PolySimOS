...
export function EvacuationStudio() {
  ...
  const exitList = () => { const n = Math.round(exitsRef.current); const arr: [number, number][] = []; for (let i = 0; i < n; i++) arr.push([W - 4, (H * (i + 1)) / (n + 1)]); return arr; };
  ...
  const frame = (steps: number) => {
    ...
    for (let s = 0; s < steps; s++) {
      ...
      for (const a of A) { if (a.out) { done++; continue; }
        ...
        const vx = dx * spd + rx * 2.2, vy = dy * spd + ry * 2.2;
  ...
  return (
    <StudioChrome title="Building Evacuation / Egress" tagline="crowd flow through exits"
      controls={<div>
        ...
        <div className="mt-3"><TransportBar playing={t.playing} onToggle={t.toggle} onStep={t.step} onReset={() => { reset(); t.step(); }} speed={t.speed} onSpeed={t.setSpeed} /></div>
