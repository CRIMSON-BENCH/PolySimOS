  ...
  return (
    <StudioChrome title="RSA Public-Key Encryption" tagline="the math behind secure internet"
      controls={<div>
        ...
        <input type="range" min={2} max={Math.max(3, n - 1)} value={message} onChange={(ev) => setMessage(+ev.target.value)} className="w-full" />
      ...
      inspector={<div><Stat label="Modulus n = p·q" value={String(n)} /><Stat label="φ(n)" value={String(phi)} /><Stat label="Public key e" value={String(e)} /><Stat label="Private key d" value={String(d)} /></div>}
