...
export function HurricaneStudio() {
  ...
  const deficitRef = useRef(deficit); deficitRef.current = deficit;
  const catRef = useRef(cat); catRef.current = cat;

  const frame = (steps: number) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    rot.current += (0.02 + deficitRef.current / 5000) * steps; const W = 400, H = 380; const ctx = hidpi(canvas, W, H); ctx.fillStyle = "#020617"; ctx.fillRect(0, 0, W, H);
    const cx = W / 2, cy = H / 2; const size = 60 + deficitRef.current * 1.2;
    // spiral bands
    ...
    ctx.fillStyle = "#94a3b8"; ctx.font = "11px sans-serif"; ctx.fillText(`Category ${catRef.current} — eye at center`, 12, 20);
  };
